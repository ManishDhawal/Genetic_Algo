...
typedef struct Chrom                  //chrom structure
   {
    short int bit[6];
      int fit;
   }chrom;                            // chrom type
...
void main()
{
   ...
   chrom popcurrent[4];   //4 chroms in popcurrent
   chrom popnext[4];      //4 chroms in popnext

   enter: printf("\nPlease enter the no. of iterations:  ");
   scanf("%d",&num);            // enter the number in numericals only
   ...
   for(i=0;i<num;i++)                      	// loop num times
{
   ...
   getch();                                	// wait for a character from the keyboard to end
